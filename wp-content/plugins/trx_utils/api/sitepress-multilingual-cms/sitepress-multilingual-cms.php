...
if ( !function_exists( 'trx_utils_wpml_get_default_language' ) ) {
    function trx_utils_wpml_get_default_language() {
        return trx_utils_exists_wpml() ? apply_filters( 'wpml_default_language', null ) : '';
    }
}

// Return current language
if ( !function_exists( 'trx_utils_wpml_get_current_language' ) ) {
    function trx_utils_wpml_get_current_language() {
        return trx_utils_exists_wpml() ? apply_filters( 'wpml_current_language', null ) : '';
    }
}


// Create option with current language
if (!function_exists('trx_utils_wpml_add_current_language_option')) {
    add_filter('trx_utils_filter_options', 'trx_utils_wpml_add_current_language_option');
    function trx_utils_wpml_add_current_language_option($options) {
        if (trx_utils_exists_wpml()) {
            $options['wpml_current_language'] = array(
                "title" => '',
                "desc" => '',
                "std" => trx_utils_wpml_get_current_language(),
                "type" => "hidden"
            );
        }
        return $options;
...
if (!function_exists('trx_utils_wpml_replace_translated_options')) {
    add_filter('trx_utils_filter_load_options', 'trx_utils_wpml_replace_translated_options');
    function trx_utils_wpml_replace_translated_options($values) {
        if (trx_utils_exists_wpml()) {
            global $trx_utils_STORAGE;
            if (is_array($values) && isset($trx_utils_STORAGE['options']) && is_array($trx_utils_STORAGE['options'])) {
                $translated = apply_filters('trx_utils_filter_load_options_translated', get_option('trx_utils_options_translated'));
                if (empty($translated)) $translated = array();
                $lang = trx_utils_wpml_get_current_language();
                foreach ($trx_utils_STORAGE['options'] as $k=>$v) {
...
// Disable menu cache if WPML is active
if (!function_exists('trx_utils_wpml_disable_menu_cache')) {
    add_filter('trx_utils_filter_options_save', 'trx_utils_wpml_disable_menu_cache');
    function trx_utils_wpml_disable_menu_cache($values) {
        if (trx_utils_exists_wpml()) {
            if (!empty($values['menu_cache'])) $values['menu_cache'] = 0;
        }
        return $values;
...
if (!function_exists('trx_utils_wpml_duplicate_options')) {
    add_filter('trx_utils_filter_options_save', 'trx_utils_wpml_duplicate_options');
    function trx_utils_wpml_duplicate_options($values) {
        if (trx_utils_exists_wpml()) {
            ...
            if (is_array($values)) {
                ...
                if (is_array($values) && isset($trx_utils_STORAGE['options']) && is_array($trx_utils_STORAGE['options'])) {
                    ...
                    if ($changed) {
                        update_option('trx_utils_options_translated', $translated);
                    }
                }
            }
        }
...
if ( !function_exists( 'trx_utils_wpml_importer_check_options' ) ) {
    if (is_admin()) add_filter( 'trx_utils_filter_import_theme_options',	'trx_utils_wpml_importer_check_options', 10, 4 );
    function trx_utils_wpml_importer_check_options($allow, $k, $v, $options) {
        if ($allow && $k == 'icl_sitepress_settings') {
            $allow = trx_utils_exists_wpml() && in_array('sitepress-multilingual-cms', $options['required_plugins']);
        }
        return $allow;
    }
}
...
if ( !function_exists( 'trx_utils_wpml_importer_show_params' ) ) {
    if (is_admin()) add_action( 'trx_utils_action_importer_params',	'trx_utils_wpml_importer_show_params', 10, 1 );
    function trx_utils_wpml_importer_show_params($importer) {
        if ( trx_utils_exists_wpml() && in_array('sitepress-multilingual-cms', $importer->options['required_plugins']) ) {
            $importer->show_importer_params(array(
                'slug' => 'sitepress-multilingual-cms',
                'title' => esc_html__('Import Sitepress Multilingual CMS (WPML)', 'trx_utils'),
                'part' => 0
            ));
        }
    }
...
// Create tables
if ( !function_exists( 'trx_utils_wpml_importer_create_tables' ) ) {
    if (is_admin()) add_action( 'trx_utils_action_importer_clear_tables',	'trx_utils_wpml_importer_create_tables', 10, 2 );
    function trx_utils_wpml_importer_create_tables() {
        global $wpdb;
...
if ( !function_exists( 'trx_utils_wpml_importer_clear_tables' ) ) {
    if (is_admin()) add_action( 'trx_utils_action_importer_clear_tables',	'trx_utils_wpml_importer_clear_tables', 10, 2 );
    function trx_utils_wpml_importer_clear_tables($importer, $clear_tables) {
        if ( trx_utils_exists_wpml() && in_array('sitepress-multilingual-cms', $importer->options['required_plugins']) ) {
            trx_utils_wpml_importer_create_tables();
...
// Import posts
if ( !function_exists( 'trx_utils_wpml_importer_import' ) ) {
    if (is_admin()) add_action( 'trx_utils_action_importer_import',	'trx_utils_wpml_importer_import', 10, 2 );
    function trx_utils_wpml_importer_import($importer, $action) {
        if ( trx_utils_exists_wpml() && in_array('sitepress-multilingual-cms', $importer->options['required_plugins']) ) {
            if ( $action == 'import_sitepress-multilingual-cms' ) {

                $importer->response['start_from_id'] = 0;
                $importer->import_dump('sitepress-multilingual-cms', esc_html__('Sitepress Multilingual CMS (WPML) data', 'trx_utils'));
            }
        }
    }
}
...
if ( !function_exists( 'trx_utils_wpml_importer_import_fields' ) ) {
    if (is_admin()) add_action( 'trx_utils_action_importer_import_fields',	'trx_utils_wpml_importer_import_fields', 10, 1 );
    function trx_utils_wpml_importer_import_fields($importer) {
        if ( trx_utils_exists_wpml() && in_array('sitepress-multilingual-cms', $importer->options['required_plugins']) ) {
            $importer->show_importer_fields(array(
                    'slug'=>'sitepress-multilingual-cms',
                    'title' => esc_html__('Sitepress Multilingual CMS (WPML) data', 'trx_utils')
                )
            );
        }
    }
...
// Export posts
if ( !function_exists( 'trx_utils_wpml_importer_export' ) ) {
    if (is_admin()) add_action( 'trx_utils_action_importer_export',	'trx_utils_wpml_importer_export', 10, 1 );
    function trx_utils_wpml_importer_export($importer) {
        if ( trx_utils_exists_wpml() && in_array('sitepress-multilingual-cms', $importer->options['required_plugins']) ) {
            trx_utils_fpc($importer->export_file_dir('sitepress-multilingual-cms.txt'), serialize( array(
...
if ( !function_exists( 'trx_utils_wpml_importer_export_fields' ) ) {
    if (is_admin()) add_action( 'trx_utils_action_importer_export_fields',	'trx_utils_wpml_importer_export_fields', 10, 1 );
    function trx_utils_wpml_importer_export_fields($importer) {
        if ( trx_utils_exists_wpml() && in_array('sitepress-multilingual-cms', $importer->options['required_plugins']) ) {
