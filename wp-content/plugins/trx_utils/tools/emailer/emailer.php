...
class themerex_emailer {
	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			themerex_admin_add_menu_item('management', array(
	...
	function build_page() {
		...
		if ( isset($_POST['emailer_subject']) ) {
			do {
				...
				$this->success = esc_html__('E-Mail was send successfull!', 'trx_utils') . $add_msg;
			} while (false);
		}
						...
						<div class="trx_emailer_fields">
							<div class="trx_emailer_field trx_emailer_group">
								...
								<select name="emailer_group" id="emailer_group">
									<option value="none"<?php echo balanceTags($group=='none' ? ' selected="selected"' : ''); ?>><?php esc_html_e('- Select group -', 'trx_utils'); ?></option>
									<?php
								...
								<input type="checkbox" name="emailer_subscribers_update" id="emailer_subscribers_update" value="1"<?php echo balanceTags($subscribers_update ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_update" class="inline" title="<?php esc_html_e('Update the subscribers list for selected group', 'trx_utils'); ?>"><?php esc_html_e('Update', 'trx_utils'); ?></label>
								<input type="checkbox" name="emailer_subscribers_clear" id="emailer_subscribers_clear" value="1"<?php echo balanceTags($subscribers_clear ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_clear" class="inline" title="<?php esc_html_e('Clear this group from not confirmed emails after send', 'trx_utils'); ?>"><?php esc_html_e('Clear', 'trx_utils'); ?></label>
								<input type="checkbox" name="emailer_subscribers_delete" id="emailer_subscribers_delete" value="1"<?php echo balanceTags($subscribers_delete ? ' checked="checked"' : ''); ?> /><label for="emailer_subscribers_delete" class="inline" title="<?php esc_html_e('Delete this group after send', 'trx_utils'); ?>"><?php esc_html_e('Delete', 'trx_utils'); ?></label>
							</div>
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email])) {
				if ($subscribers[$group][$email] == $nonce) {
					...
					update_option('themerex_emailer_subscribers', $subscribers);
					themerex_set_system_message(esc_html__('Confirmation complete! E-mail address succefully added in the subscribers list!', 'trx_utils'), 'success');
		
					wp_safe_redirect( home_url('/') );
				} else if ($subscribers[$group][$email] != '') {
					themerex_set_system_message(esc_html__('Bad confirmation code!', 'trx_utils'), 'error');
				
					wp_safe_redirect( home_url('/') );
				} else {
					themerex_set_system_message(esc_html__('E-mail address already exists in the subscribers list!', 'trx_utils'), 'error');
