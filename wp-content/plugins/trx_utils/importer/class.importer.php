...
class trx_utils_demo_data_importer {
	...
	function admin_scripts() {
        ...
        wp_enqueue_style( 'fontello-style',  TRX_UTILS_PLUGIN_URL . 'importer/css/fontello/css/fontello.css',  array(), null);
	...
	function build_page() {
		...
		?><div class="trx_importer"><?php
			...
			?><div class="trx_importer_section">
				...
				<p class="trx_importer_description"><?php echo wp_kses_data(__('<b>Attention! Some plugins</b> (for example BuddyPress and bbPress) <b>increases the memory consumption</b> at 60-80MB depending on the configuration. So if you have problems with installing the demo-data due to lack of memory - deactivate some plugins and then retry the installation of the demo-data.')); ?></p>
    ...
    function export_file_dir($fname) {
        return trx_utils_get_file_dir(TRX_UTILS_PLUGIN_IMPORTER . "export/{$fname}");
    }

    // Return url to the file in the 'export' directory
    function export_file_url($fname) {
        return trx_utils_get_file_url(TRX_UTILS_PLUGIN_IMPORTER . "export/{$fname}");
    }
    ...
    function import_theme_options() {
        if ($this->options['debug'])
            dfl(__('Import Plugins Settings', 'trx_utils'));
        ...
        if (is_array($data) && count($data) > 0) {
            foreach ($data as $k=>$v) {
                if (apply_filters('trx_utils_filter_import_theme_options', true, $k, $v, $this->options)) {
                    update_option( $k, apply_filters('trx_utils_filter_import_theme_options_value', $this->replace_uploads($v), $k) );
                }
            }
        } else {
            if ($this->options['debug'])
