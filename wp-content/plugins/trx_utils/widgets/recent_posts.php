...
class themerex_widget_recent_posts extends WP_Widget {
	...
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'themerex_widget_recent_posts', esc_html__('ThemeREX - Recent Posts', 'trx_utils'), $widget_ops, $control_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'trx_utils')), themerex_get_list_blog_counters(false));
			...
			<?php
				if (is_array($counters) && count($counters) > 0) {
					foreach ($counters as $id => $title) {
						?>
						<input type="radio" id="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($this->get_field_name('show_counters')); ?>" value="<?php echo esc_attr($id); ?>" <?php echo balanceTags($show_counters==$id ? ' checked="checked"' : ''); ?> />
