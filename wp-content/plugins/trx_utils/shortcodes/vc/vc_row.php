...
if (function_exists('vc_map_get_attributes')) {						// New VC
	...
	// Add ThemeREX color scheme in the row
	if ($scheme && !themerex_param_is_off($scheme) && !themerex_param_is_inherit($scheme)) $css_classes[] = 'scheme_'.esc_attr($scheme);
	...
	// Add ThemeREX color scheme in the row
	if ($scheme && !themerex_param_is_off($scheme) && !themerex_param_is_inherit($scheme)) $css_class .= ' scheme_'.esc_attr($scheme);
	...
	} ?> <?php echo balanceTags($style); ?>><?php
	echo wpb_js_remove_wpautop( $content );
	?></div><?php echo balanceTags($this->endBlockComment( 'row' ));
	if ( ! empty( $full_width ) ) {
