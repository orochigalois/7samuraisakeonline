...
<html <?php language_attributes(); ?> class="<?php
    echo esc_attr('scheme_'.$body_scheme); ?>">
<head>
	<?php wp_head(); ?>
</head>
...
<body <?php body_class();	?>>
	...
	if (themerex_get_custom_option('menu_toc_home')=='yes')
        if (function_exists('themerex_sc_anchor'))
		themerex_show_layout(themerex_sc_anchor(array(
			'id' => "toc_home",
			'title' => esc_html__('Home', 'wineshop'),
			'description' => esc_html__('{{Return to Home}} - ||navigate to home page of the site', 'wineshop'),
			'icon' => "icon-home",
			'separator' => "yes",
			'url' => home_url('/'))
			)); 
	if (themerex_get_custom_option('menu_toc_top')=='yes')
        if (function_exists('themerex_sc_anchor'))
		themerex_show_layout(themerex_sc_anchor(array(
			'id' => "toc_top",
			'title' => esc_html__('To Top', 'wineshop'),
			'description' => esc_html__('{{Back to top}} - ||scroll to top of the page', 'wineshop'),
			'icon' => "icon-double-up",
			'separator' => "yes")
			)); 
	?>
	...
	<?php
		$class = $style = '';
			...
			if (($img = (int) themerex_get_custom_option('bg_image', 0)) > 0)
				$class = 'bg_image_'.($img);
			else if (($img = (int) themerex_get_custom_option('bg_pattern', 0)) > 0)
				$class = 'bg_pattern_'.($img);
			else if (($img = themerex_get_custom_option('bg_color', '')) != '')
		...
		<div class="page_wrap">
			...
			if ($show_title || $show_breadcrumbs) {
				...
				<div class="top_panel_title top_panel_style_<?php echo esc_attr(str_replace('header_', '', $top_panel_style)); ?> <?php themerex_show_layout($show_title ? ' title_present' : '' . ($show_breadcrumbs ? ' breadcrumbs_present' : '')); ?> scheme_<?php echo esc_attr($top_panel_scheme); ?>">
					<div class="top_panel_title_inner top_panel_inner_style_<?php echo esc_attr(str_replace('header_', '', $top_panel_style)); ?> <?php themerex_show_layout($show_title ? ' title_present_inner' : '' . ($show_breadcrumbs ? ' breadcrumbs_present_inner' : '')); ?>" <?php echo (themerex_get_custom_option('page_title_image') != '' ? 'style="background: url('.themerex_get_custom_option('page_title_image').');"' : ''); ?>>
						<div class="content_wrap">
							<?php if ($show_title) { ?>
								<h1 class="page_title">
                                    <?php
                                    echo wp_kses_post(themerex_get_blog_title());
                                    ?>
                                </h1>
							<?php } ?>
			...
			<div class="page_content_wrap page_paddings_<?php echo esc_attr(themerex_get_custom_option('body_paddings')); ?>">
