...
if ( !function_exists( 'themerex_template_events_2_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_events_2_theme_setup', 1 );
	function themerex_template_events_2_theme_setup() {
		themerex_add_template(array(
			'layout' => 'events-2',
			'template' => 'events-2',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 2/', 'wineshop')
		));
	}
}

// Template output
if ( !function_exists( 'themerex_template_events_2_output' ) ) {
	function themerex_template_events_2_output($post_options, $post_data) {
		$show_title = true;
		...
		<div<?php themerex_show_layout($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
			...
			<span class="sc_events_item_date">
				<span class="sc_events_item_month"><?php themerex_show_layout($sd[0]); ?></span>
				<span class="sc_events_item_day"><?php themerex_show_layout($sd[1]); ?></span>
			</span><?php
			if ($show_title) {
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?><h6 class="sc_events_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php themerex_show_layout($post_data['post_title']); ?></a></h6><?php
				} else {
					?><h6 class="sc_events_item_title"><?php themerex_show_layout($post_data['post_title']); ?></h6><?php
				}
			}
			?><span class="sc_events_item_time"><?php
				echo (trim($start_date[1]) ? $start_date[1] : esc_html__('Whole day', 'wineshop'))
						. ($start_date[0]==$end_date[0] && trim($start_date[1]) && trim($end_date[1]) ? ' - ' . $end_date[1] : '');
			?></span><span class="sc_events_item_details"><?php
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php themerex_show_layout($post_options['readmore']); ?></a><?php
				}
			?></span>
		</div>
		<?php
