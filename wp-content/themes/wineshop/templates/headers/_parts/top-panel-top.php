...
?>
<?php if (themerex_get_custom_option('show_search')=='yes') if (function_exists('themerex_sc_search')) themerex_show_layout(themerex_sc_search(array())); ?>
<div class="top_panel_top_user_area">
	<?php
	
	if (in_array('search', $top_panel_top_components) && themerex_get_custom_option('show_search')=='yes') {
		?>
		<div class="top_panel_top_search"><?php if (function_exists('themerex_sc_search')) themerex_show_layout(themerex_sc_search(array('state'=>'fixed'))); ?></div>
		<?php
	}
	...
	if (in_array('currency', $top_panel_top_components) && function_exists('themerex_is_woocommerce_page') && themerex_is_woocommerce_page() && themerex_get_custom_option('show_currency')=='yes') {
		?>
		<li class="menu_user_currency">
			<a href="#">$</a>
			<ul>
				<li><a href="#"><b>&#36;</b> <?php esc_html_e('Dollar', 'wineshop'); ?></a></li>
				<li><a href="#"><b>&euro;</b> <?php esc_html_e('Euro', 'wineshop'); ?></a></li>
				<li><a href="#"><b>&pound;</b> <?php esc_html_e('Pounds', 'wineshop'); ?></a></li>
			</ul>
		</li>
		<?php
	}
	...
	if (in_array('bookmarks', $top_panel_top_components) && themerex_get_custom_option('show_bookmarks')=='yes') {
		...
		?>
		<li class="menu_user_bookmarks"><a href="#" class="bookmarks_show icon-star" title="<?php  esc_attr_e('Show bookmarks', 'wineshop'); ?>"><?php esc_html_e('Bookmarks', 'wineshop'); ?></a>
		<?php 
			$list = themerex_get_value_gpc('themerex_bookmarks', '');
			if (!empty($list)) $list = json_decode($list, true);
			?>
			<ul class="bookmarks_list">
				<li><a href="#" class="bookmarks_add icon-star-empty" title="<?php  esc_attr_e('Add the current page into bookmarks', 'wineshop'); ?>"><?php esc_html_e('Add bookmark', 'wineshop'); ?></a></li>
				<?php 
				if (!empty($list) && is_array($list)) {
					foreach ($list as $bm) {
						echo '<li><a href="'.esc_url($bm['url']).'" class="bookmarks_item">'.($bm['title']).'<span class="bookmarks_delete icon-cancel" title="'.esc_attr__('Delete this bookmark', 'wineshop').'"></span></a></li>';
					}
				}
				?>
			</ul>
           ...
           themerex_enqueue_messages();
			...
			<li class="menu_user_controls">
				...
				<ul>
					...
					<li><a href="<?php echo esc_url(home_url('/')); ?>/wp-admin/post-new.php?post_type=post" class="icon icon-doc"><?php esc_html_e('New post', 'wineshop'); ?></a></li>
					<?php } ?>
					<li><a href="<?php echo esc_url(get_edit_user_link()); ?>" class="icon icon-cog"><?php esc_html_e('Settings', 'wineshop'); ?></a></li>
				</ul>
			</li>
			<li class="menu_user_logout"><a href="<?php echo esc_url(wp_logout_url(home_url('/'))); ?>" class="icon icon-logout"><?php esc_html_e('Logout', 'wineshop'); ?></a></li>
			<?php 
		}
	}

	if (in_array('cart', $top_panel_top_components) && function_exists('themerex_exists_woocommerce') && themerex_exists_woocommerce() && (themerex_is_woocommerce_page() && themerex_get_custom_option('show_cart')=='shop' || themerex_get_custom_option('show_cart')=='always') && !(is_checkout() || is_cart() || defined('WOOCOMMERCE_CHECKOUT') || defined('WOOCOMMERCE_CART'))) { 
		?>
