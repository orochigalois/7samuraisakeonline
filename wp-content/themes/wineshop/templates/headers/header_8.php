...
if ( !function_exists( 'themerex_template_header_8_theme_setup' ) ) {
	...
	function themerex_template_header_8_theme_setup() {
		themerex_add_template(array(
			'layout' => 'header_8',
			'mode'   => 'header',
			'title'  => esc_html__('Header 8', 'wineshop'),
				...
				<div class="top_panel_middle" <?php themerex_show_layout($header_css); ?>>
					<div class="content_wrap">
						...
						<div class="menu_pushy_wrap clearfix">
							<a href="#" class="menu_pushy_button icon-menu"><?php esc_html_e('MENU', 'wineshop'); ?></a>
						</div>
					</div>
				</div>
			...
			<div class="pushy_inner">
					...
					<div class="contact_info">
						<?php if (!empty($address_1) || !empty($address_2)) { ?>
							<address class="contact_address">
								<?php themerex_show_layout(($address_1) . (!empty($address_1) ? ', ' : '') . ($address_2)); ?>
							</address>
						<?php } ?>
						<?php if (!empty($phone) || !empty($fax)) { ?>
							<address class="contact_phones">
								<?php echo esc_html__('Call:', 'wineshop') . ' ' . ($phone) . (!empty($phone) ? ', ' : '') . ($fax); ?>
							</address>
						<?php } ?>
					</div>
					<?php
				...
				if (themerex_get_custom_option('show_socials')=='yes') {
					...
					<div class="contact_socials">
                       <?php if (function_exists('themerex_sc_socials'))
						themerex_show_layout(themerex_sc_socials(array('size'=>'small'))); ?>
					</div>
					<?php
				}
				?>
