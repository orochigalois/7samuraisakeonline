...
if ( !function_exists( 'themerex_template_single_standard_theme_setup' ) ) {
	...
	function themerex_template_single_standard_theme_setup() {
		themerex_add_template(array(
			...
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'wineshop'),
			'thumb_title'  => esc_html__('Fullwidth image', 'wineshop'),
			'w'		 => 1170,
			'h'		 => 660
		));
	}
...
if ( !function_exists( 'themerex_template_single_standard_output' ) ) {
	function themerex_template_single_standard_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && themerex_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="<?php themerex_show_layout($avg_author > 0 || $avg_users > 0 ? 'itemReviewed' : 'headline'); ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php themerex_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
		<?php 
		}
		...
		if ($post_data['post_protected']) {
            ...
            if (function_exists('themerex_sc_reviews'))
                if (themerex_strpos($post_data['post_content'], themerex_get_reviews_placeholder()) === false) $post_data['post_content'] = themerex_sc_reviews(array()) . ($post_data['post_content']);
			...
			if ( themerex_get_custom_option('show_post_tags') == 'yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				...
				<div class="post_info post_info_bottom">
					<span class="post_info_item post_info_tags"><?php esc_html_e('Tags:', 'wineshop'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php 
			}
		} 
