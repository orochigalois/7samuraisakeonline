...
if ( !function_exists( 'themerex_template_services_3_theme_setup' ) ) {
	...
	function themerex_template_services_3_theme_setup() {
		themerex_add_template(array(
			...
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 3/', 'wineshop'),
	...
	function themerex_template_services_3_output($post_options, $post_data) {
		...
		?>
			<div<?php themerex_show_layout($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				class="sc_services_item sc_services_item_<?php echo esc_attr($post_options['number']) . ($post_options['number'] % 2 == 1 ? ' odd' : ' even') . ($post_options['number'] == 1 ? ' first' : '') . (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?> <?php echo (!$show_title || $post_data['post_title'] == '' ? 'without_title' : 'with_title'); ?>"
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					...
					if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
						?><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php themerex_show_layout(themerex_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]')); ?></a><?php
					} else
						themerex_show_layout(themerex_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
