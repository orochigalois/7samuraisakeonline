...
if ( !function_exists( 'themerex_template_polaroid_theme_setup' ) ) {
	...
	function themerex_template_polaroid_theme_setup() {
		themerex_add_template(array(
			...
			'title'  => esc_html__('Blogger layout: Polaroid', 'wineshop'),
...
if ( !function_exists( 'themerex_template_polaroid_output' ) ) {
	function themerex_template_polaroid_output($post_options, $post_data) {
		...
		?>
		<figure class="post_item sc_blogger_item sc_polaroid_item<?php themerex_show_layout($post_options['number'] == $post_options['posts_on_page'] && !themerex_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : ''); ?>">
			<a href="<?php echo esc_url($post_data['post_link']); ?>" class="photostack-img"><?php themerex_show_layout($post_data['post_thumb']); ?></a>
			<figcaption class="photostack_info">
				<h6 class="post_title sc_title sc_blogger_title sc_polaroid_title photostack-title"><?php themerex_show_layout($post_data['post_title']); ?></h6>
				<?php
