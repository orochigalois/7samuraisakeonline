...
if ($show_date || $show_counters || $show_author) {
	...
	if ($show_author) {
		...
		$output .= '<span class="post_info_item post_info_posted_by">' . esc_html__('by', 'wineshop') . ' ' . ($post_link ? '<a href="' . esc_url($post_author_url) . '" class="post_info_author">' : '') . ($post_author_name) . ($post_link ? '</a>' : '') . '</span>';
	}
	if ($show_counters && themerex_strpos($show_counters, 'stars')===false) {
		...
		$output .= '<span class="post_info_item post_info_counters">'
			. ($post_counters_link ? '<a href="' . esc_url($post_counters_link) . '"' : '<span') 
				...
				. (themerex_strpos($show_counters, 'likes')!==false
					? ' title="' . ($allow ? esc_attr__('Like', 'wineshop') : esc_attr__('Dislike', 'wineshop')) . '"'
						. ' data-postid="' . esc_attr($post_id) . '"'
                        . ' data-likes="' . esc_attr($post_counters) . '"'
                        . ' data-title-like="' . esc_attr__('Like', 'wineshop') . '"'
                        . ' data-title-dislike="' . esc_attr__('Dislike', 'wineshop') . '"'
					: ''
				)
				. '>'
