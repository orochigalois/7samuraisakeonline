...
if ($avg_author > 0 || $avg_users > 0) {
	...
	$output = $marks = $users = '';
	if ($use_tabs) {
		$author_tab = '<li class="sc_tabs_title"><a href="#author_marks" class="theme_button">'.esc_html__('Author', 'wineshop').'</a></li>';
		$users_tab = '<li class="sc_tabs_title"><a href="#users_marks" class="theme_button">'.esc_html__('Users', 'wineshop').'</a></li>';
	...
	if (!$reviews_first_author || !$reviews_second_hide) {
		...
		$field["descr"] = wp_kses( sprintf(__("Summary rating from <b>%s</b> user's marks.", 'wineshop'), $users)
									. ' ' 
                                    . ( !isset($_COOKIE['themerex_votes']) || themerex_strpos($_COOKIE['themerex_votes'], ','.($post_data['post_id']).',')===false
											? esc_html__ ('You can set own marks for this article - just click on stars above and press "Accept".', 'wineshop')
                                            : esc_html__ ('Thanks for your vote!', 'wineshop')
                                      ), $THEMEREX_GLOBALS['allowed_tags'] );
	...
	if ($allow_user_marks) {
        ...
        themerex_storage_set_array('js_vars', 'reviews_allow_user_marks', $allow_user_marks);
        themerex_storage_set_array('js_vars', 'reviews_max_level', $max_level);
        themerex_storage_set_array('js_vars', 'reviews_levels', themerex_get_theme_option('reviews_criterias_levels'));
        themerex_storage_set_array('js_vars', 'reviews_vote', isset($_COOKIE['themerex_votes']) ? $_COOKIE['themerex_votes'] : '');
        themerex_storage_set_array('js_vars', 'reviews_marks', explode(',', $marks));
        themerex_storage_set_array('js_vars', 'reviews_users', max(0, $users));
        themerex_storage_set_array('js_vars', 'post_id', $post_data['post_id']);
	}
	global $THEMEREX_GLOBALS;
