...
if ( !function_exists( 'themerex_true_load_language' ) ) {
    add_action('after_setup_theme', 'themerex_true_load_language');
    function themerex_true_load_language()
    {
        load_theme_textdomain('wineshop', get_template_directory() . '/languages');
    }
}
...
if ( !function_exists( 'themerex_core_theme_setup' ) ) {
	...
	function themerex_core_theme_setup() {
		...
		load_theme_textdomain( 'wineshop', themerex_get_folder_dir('languages') );
...
if ( !function_exists( 'themerex_register_theme_menus' ) ) {
	function themerex_register_theme_menus() {
		register_nav_menus(apply_filters('themerex_filter_add_theme_menus', array(
			'menu_main'		=> esc_html__('Main Menu', 'wineshop'),
			'menu_user'		=> esc_html__('User Menu', 'wineshop'),
			'menu_footer'	=> esc_html__('Footer Menu', 'wineshop'),
			'menu_side'		=> esc_html__('Side Menu', 'wineshop')
		)));
	}
}
...
if ( !function_exists( 'themerex_core_frontend_scripts' ) ) {
	function themerex_core_frontend_scripts() {
        ...
        wp_enqueue_script( 'modernizr', themerex_get_file_url('js/photostack/modernizr.min.js'), array(), null, false );
		...
		if (is_array($theme_fonts) && count($theme_fonts) > 0) {
			...
			foreach ($theme_fonts as $font=>$v) {
				if (isset($fonts[$font])) {
					...
					if (!empty($fonts[$font]['css'])) {
						$css = $fonts[$font]['css'];
                        wp_enqueue_style( 'themerex-font-'.str_replace(' ', '-', $font_name).'-style', $css, array(), null );
					} else {
            ...
            if ($google_fonts) {
                ...
                */
                $google_fonts_enabled = ( 'off' !== esc_html_x( 'on', 'Google fonts: on or off', 'wineshop' ) );
                if ( $google_fonts_enabled ) {
                    wp_enqueue_style( 'themerex-font-google-fonts-style', add_query_arg( 'family', $google_fonts . '&subset=' . themerex_get_theme_option('fonts_subset'), "//fonts.googleapis.com/css" ), array(), null );

                }
            }
		}

        wp_enqueue_style( 'themerex-font-google-satisfy', themerex_get_protocol() . '://fonts.googleapis.com/css?family=Satisfy', array(), null );
		...
		if (themerex_get_theme_option('responsive_layouts') == 'yes') {
            ...
            wp_enqueue_style( 'themerex-responsive-addition-style', themerex_get_file_url('includes/css/responsive_addition.css'), array(), null );
		...
		if ( function_exists('themerex_reviews_theme_setup') && is_single() && themerex_get_custom_option('show_reviews')=='yes' ) {
        ...
        wp_enqueue_script( 'themerex-theme-init-script',	themerex_get_file_url('js/theme.init.js'), array('jquery'), null, true );
        ...
        // Comments
        if ( is_singular() && !themerex_storage_get('blog_streampage') && comments_open() && get_option( 'thread_comments' ) ) {
...
if ( !function_exists( 'themerex_enqueue_slider' ) ) {
	function themerex_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
            ...
            wp_enqueue_script( 'swiperslider-script', 			themerex_get_file_url('js/swiper/swiper.js'), array(), null, true );
		}
	}
}
...
if ( !function_exists( 'themerex_enqueue_polaroid' ) ) {
	function themerex_enqueue_polaroid() {
        wp_enqueue_style(  'polaroid-style', 	themerex_get_file_url('js/photostack/component.css'), array(), null );
        wp_enqueue_script( 'classie-script',		themerex_get_file_url('js/photostack/classie.js'), array(), null, true );
        wp_enqueue_script( 'polaroid-script',	themerex_get_file_url('js/photostack/photostack.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'themerex_enqueue_diagram' ) ) {
	function themerex_enqueue_diagram($type='all') {
		if ($type=='all' || $type=='pie') wp_enqueue_script( 'chart-script',	themerex_get_file_url('js/diagram/chart.min.js'), array(), null, true );
		if ($type=='all' || $type=='arc') wp_enqueue_script( 'raphael-script',	themerex_get_file_url('js/diagram/diagram.raphael.min.js'), array(), 'no-compose', true );
	}
}
...
if (!function_exists('themerex_core_add_scripts_inline')) {
    function themerex_core_add_scripts_inline($vars = array()) {
        ...
        $vars['menu_mobile'] = themerex_get_theme_option('responsive_layouts') == 'yes' ? max(0, (int) themerex_get_theme_option('menu_mobile')) : 0;
        ...
        // Menu cache is used
        $vars['menu_cache']    = themerex_get_theme_option('use_menu_cache')=='yes';
...
if ( !function_exists( 'themerex_wp_title' ) ) {
	function themerex_wp_title( $title, $sep ) {
		...
		if ( $paged >= 2 || $page >= 2 )
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'wineshop' ), max( $paged, $page ) );
		return $title;
	}
}
...
if ( !function_exists( 'themerex_callback_frontend_editor_save' ) ) {
	function themerex_callback_frontend_editor_save() {
		...
		parse_str(themerex_get_value_gpc('data'), $output);
		...
		if ( themerex_get_theme_option("allow_editor")=='yes' && (current_user_can('edit_posts', $post_id) || current_user_can('edit_pages', $post_id)) ) {
			if ($post_id > 0) {
				...
				if ($rez == 0) 
					$response['error'] = esc_html__('Post update error!', 'wineshop');
			} else {
				$response['error'] = esc_html__('Post update error!', 'wineshop');
			}
		} else
			$response['error'] = esc_html__('Post update denied!', 'wineshop');
	...
	function themerex_callback_frontend_editor_delete() {
		...
		if ( themerex_get_theme_option("allow_editor")=='yes' && (current_user_can('delete_posts', $post_id) || current_user_can('delete_pages', $post_id)) ) {
			if ($post_id > 0) {
				...
				if ($rez === false) 
					$response['error'] = esc_html__('Post delete error!', 'wineshop');
			} else {
				$response['error'] = esc_html__('Post delete error!', 'wineshop');
			}
		} else
			$response['error'] = esc_html__('Post delete denied!', 'wineshop');
