...
if (!function_exists('themerex_get_css_position_as_classes')) {
	function themerex_get_css_position_as_classes($top='',$right='',$bottom='',$left='') {
		if (!is_array($top)) {
		...
		if (is_array($top) && count($top) > 0) {
			foreach ($top as $k=>$v) {
				if (!empty($v) && !themerex_param_is_inherit($v)) $output .= ($output ? ' ' : '') . 'margin_'.esc_attr($k) . '_' . esc_attr($v);
			}
		}
		return $output;
	}
		...
		if (is_array($top) && count($top) > 0) {
			foreach ($top as $k=>$v) {
				...
				if ($v != '') $output .= ($k=='width' 
											...
											) . ':' . esc_attr(themerex_prepare_css_value($v)) . ($imp=='!' ? ' !important' : '') . ';';
			}
		}
		return $output;
	}
...
if (!function_exists('themerex_html_decode')) {
	function themerex_html_decode($prm) {
		if (is_array($prm) && count($prm) > 0) {
			foreach ($prm as $k=>$v) {
				if (is_string($v))
