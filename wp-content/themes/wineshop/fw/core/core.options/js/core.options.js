...
function themerex_options_action_media_upload(obj) {
	...
	THEMEREX_GLOBALS['to_media_frame'][fieldId]['frame'] = wp.media(media_args);
			...
			if (type=='text' || type=='textarea' || type=='hidden' || type=='spinner') {
				...
				if (val[i]!='') {
					...
					if (file!='') {
						...
						fld.eq(cnt).after('<a class="themerex_options_image_preview" rel="prettyPhoto" target="_blank" href="'+val[i]+'">'+('jpg,png,gif'.indexOf(ext)>=0 ? '<img src="'+val[i]+'" alt="Image" />' : '<span>'+fname+'</span>')+'</a>');
					}
				}
			} else if (type=='range') {
				fld.eq(cnt).val(val[i]).trigger('change');
