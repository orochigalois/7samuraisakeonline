...
if ( !function_exists( 'themerex_options_admin_menu_item' ) ) {
	function themerex_options_admin_menu_item() {
		...
		themerex_admin_add_menu_item('menu', array(
			'page_title' => esc_html__('Global Options', 'wineshop'),
			'menu_title' => esc_html__('Theme Options', 'wineshop'),
		...
		themerex_admin_add_menu_item('submenu', array(
			...
			'page_title' => esc_html__('Global Options', 'wineshop'),
			'menu_title' => esc_html__('Global Options', 'wineshop'),
			'capability' => 'manage_options',
			'menu_slug'  => 'themerex_options',
			'callback'   => 'themerex_options_page',
			'icon'		 => ''
		...
		if (!empty($inheritance) && is_array($inheritance)) {
			foreach($inheritance as $k=>$v) {
				...
				if (!empty($v['stream_template'])) {
					...
					$title = !empty($v['stream_template_title']) ? $v['stream_template_title'] : themerex_strtoproper(str_replace('_', ' ', $slug));
					themerex_admin_add_menu_item('submenu', array(
						'parent'     => 'themerex_options',
						'page_title' => $title.' '.esc_html__('Options', 'wineshop'),
						'menu_title' => $title,
						'capability' => 'manage_options',
				...
				if (!empty($v['single_template'])) {
					...
					$title = !empty($v['single_template_title']) ? $v['single_template_title'] : themerex_strtoproper(str_replace('_', ' ', $slug));
					themerex_admin_add_menu_item('submenu', array(
						'parent'     => 'themerex_options',
						'page_title' => $title.' '.esc_html__('Options', 'wineshop'),
						'menu_title' => $title,
				...
				if (!$tpl) {
					...
					$title = !empty($v['title']) ? $v['title'] : themerex_strtoproper(str_replace('_', ' ', $slug));
					themerex_admin_add_menu_item('submenu', array(
						'parent'     => 'themerex_options',
						'page_title' => $title.' '.esc_html__('Options', 'wineshop'),
						'menu_title' => $title,
...
if ( !function_exists( 'themerex_get_options_param' ) ) {
    function themerex_get_options_param($prm) {
        return themerex_storage_get_array('options_params', $prm);
    }
}

// Set options_param value
if ( !function_exists( 'themerex_set_options_param' ) ) {
    function themerex_set_options_param($prm, $val) {
        themerex_storage_set_array('options_params', $prm, $val);
    }
}
...
// Set array element
if (!function_exists('themerex_storage_set_array')) {
    function themerex_storage_set_array($var_name, $key, $value) {
        global $THEMEREX_GLOBALS;
...
if (!function_exists('themerex_storage_concat')) {
    function themerex_storage_concat($var_name, $value) {
        global $THEMEREX_GLOBALS;
...
if (!function_exists('themerex_storage_get_array')) {
    function themerex_storage_get_array($var_name, $key, $key2='', $default='') {
        global $THEMEREX_GLOBALS;
        if (empty($key2))
            return !empty($var_name) && !empty($key) && isset($THEMEREX_GLOBALS[$var_name][$key]) ? $THEMEREX_GLOBALS[$var_name][$key] : $default;
        else
            return !empty($var_name) && !empty($key) && isset($THEMEREX_GLOBALS[$var_name][$key][$key2]) ? $THEMEREX_GLOBALS[$var_name][$key][$key2] : $default;
    }
}
...
if ( !function_exists( 'themerex_options_load_scripts' ) ) {
	function themerex_options_load_scripts() {
        ...
        wp_enqueue_script('themerex-colors-script',		themerex_get_file_url('js/colorpicker/colors.js'), array('jquery'), null, true );
        wp_enqueue_script('themerex-colorpicker-script',	themerex_get_file_url('js/colorpicker/jqColorPicker.js'), array('jquery'), null, true );
...
if ( !function_exists( 'themerex_options_prepare_scripts' ) ) {
	function themerex_options_prepare_scripts($override='') {
        ...
        themerex_storage_set_array('js_vars', 'to_options', $THEMEREX_GLOBALS['to_data'] );
        themerex_storage_set_array('js_vars', 'to_delimiter', esc_attr(themerex_storage_get('to_delimiter')));
        themerex_storage_set_array('js_vars', 'to_slug', esc_attr(themerex_storage_get_array('to_flags', 'slug')));
        themerex_storage_set_array('js_vars', 'to_popup', esc_attr(themerex_get_theme_option('popup_engine')));
        themerex_storage_set_array('js_vars', 'to_override', esc_attr($override));
        $keys = array();
        if (($export_opts = get_option(themerex_storage_get('options_prefix') . '_options_export_'.($override), false)) !== false) {
            $keys = join(',', array_keys($export_opts));
        }
        themerex_storage_set_array('js_vars', 'to_export_list', $keys);
        themerex_storage_merge_array('js_vars', 'to_strings', array(
            'del_item_error' => esc_html__("You can't delete last item! To disable it - just clear value in field.", 'wineshop'),
            'del_item' => esc_html__("Delete item error!", 'wineshop'),
            'recompile_styles' => esc_html__("When saving color schemes and font settings, recompilation of .less files occurs. It may take from 5 to 15 secs dependning on your server's speed and size of .less files.", 'wineshop'),
            'wait' => esc_html__("Please wait a few seconds!", 'wineshop'),
            'reload_page' => esc_html__("After 3 seconds this page will be reloaded.", 'wineshop'),
            'save_options' => esc_html__("Options saved!", 'wineshop'),
            'reset_options' => esc_html__("Options reset!", 'wineshop'),
            'reset_options_confirm' => esc_html__("Do you really want reset all options to default values?", 'wineshop'),
            'reset_options_complete' => esc_html__("Settings are reset to their default values.", 'wineshop'),
            'export_options_header' => esc_html__("Export options", 'wineshop'),
            'export_options_error' => esc_html__("Name for options set is not selected! Export cancelled.", 'wineshop'),
            'export_options_label' => esc_html__("Name for the options set:", 'wineshop'),
            'export_options_label2' => esc_html__("or select one of exists set (for replace):", 'wineshop'),
            'export_options_select' => esc_html__("Select set for replace ...", 'wineshop'),
            'export_empty' => esc_html__("No exported sets for import!", 'wineshop'),
            'export_options' => esc_html__("Options exported!", 'wineshop'),
            'export_link' => esc_html__("If need, you can download the configuration file from the following link: %s", 'wineshop'),
            'export_download' => esc_html__("Download theme options settings", 'wineshop'),
            'import_options_label' => esc_html__("or put here previously exported data:", 'wineshop'),
            'import_options_label2' => esc_html__("or select file with saved settings:", 'wineshop'),
            'import_options_header' => esc_html__("Import options", 'wineshop'),
            'import_options_error' => esc_html__("You need select the name for options set or paste import data! Import cancelled.", 'wineshop'),
            'import_options_failed' => esc_html__("Error while import options! Import cancelled.", 'wineshop'),
            'import_options_broken' => esc_html__("Attention! Some options are not imported:", 'wineshop'),
            'import_options' => esc_html__("Options imported!", 'wineshop'),
            'import_dummy_confirm' => esc_html__("Attention! During the import process, all existing data will be replaced with new.", 'wineshop'),
            'clear_cache' => esc_html__("Cache cleared successfull!", 'wineshop'),
            'clear_cache_header' => esc_html__("Clear cache", 'wineshop'),
        ));
	}
}
...
if ( !function_exists( 'themerex_options_page' ) ) {
	function themerex_options_page() {
		...
		themerex_options_page_start(array(
			...
			'subtitle' => empty($slug) 
								? (empty($override) 
									? esc_html__('Global Options', 'wineshop')
									: '') 
								: themerex_strtoproper(str_replace('_', ' ', $slug)) . ' ' . esc_html__('Options', 'wineshop'),
			'description' => empty($slug) 
								? (empty($override) 
									? wp_kses( __('Global settings affect the entire website\'s display. They can be overriden when editing pages/categories/posts', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] )
									: '') 
								: wp_kses( __('Settings template for a certain post type: affects the display of just one specific post type. They can be overriden when editing categories and/or posts of a certain type', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
			'slug' => $slug,
			'override' => $override
		));
...
if ( !function_exists( 'themerex_options_page_start' ) ) {
	function themerex_options_page_start($args = array()) {
		$to_flags = array_merge(array(
			'data'				=> null,
			'title'				=> esc_html__('Theme Options', 'wineshop'),	// Theme Options page title
		...
		<?php if (in_array('import', $to_flags['buttons'])) { ?>
					<div class="themerex_options_button_import"><span class="iconadmin-download"></span><?php esc_html_e('Import', 'wineshop'); ?></div>
		<?php }	?>
		<?php if (in_array('export', $to_flags['buttons'])) { ?>
					<div class="themerex_options_button_export"><span class="iconadmin-upload"></span><?php esc_html_e('Export', 'wineshop'); ?></div>
		<?php }	?>
		<?php if (in_array('reset', $to_flags['buttons'])) { ?>
					<div class="themerex_options_button_reset"><span class="iconadmin-spin3"></span><?php esc_html_e('Reset', 'wineshop'); ?></div>
		<?php }	?>
		<?php if (in_array('save', $to_flags['buttons'])) { ?>
					<div class="themerex_options_button_save"><span class="iconadmin-check"></span><?php esc_html_e('Save', 'wineshop'); ?></div>
		<?php }	?>
	...
	function themerex_options_show_field($id, $field, $value=null) {
		...
		if ($field['type']=='media') {
			...
			$field['before'] = array_merge(array(
					'title' => esc_html__('Choose image', 'wineshop'),
					'action' => 'media_upload',
					'type' => 'image',
					'multiple' => false,
					'sizes' => false,
					'linked_field' => '',
					'captions' => array('choose' => esc_html__( 'Choose image', 'wineshop'),
										'update' => esc_html__( 'Select image', 'wineshop')
										)
				), $field['before']);
					...
					echo '<label class="themerex_options_field_label'
						...
						. (!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']) 
							? '<span id="'.esc_attr($id).'_inherit" class="themerex_options_button_inherit'
								...
								.'" title="' . esc_attr__('Unlock this field', 'wineshop') . '"></span>'
							: '')
						. '</label>'
						. "\n";
				}
		...
		foreach ($field['val'] as $clone_num => $clone_val) {
			...
			case 'media':
				...
				if (!empty($clone_val) && !themerex_is_inherit_option($clone_val)) {
					...
					echo '<a class="themerex_options_image_preview" data-rel="popup" target="_blank" href="'.esc_url($clone_val).'">'.(!empty($ext) && themerex_strpos('jpg,png,gif', $ext)!==false ? '<img src="'.esc_url($clone_val).'" alt="'.esc_attr__('Image', 'wineshop').'" />' : '<span>'.($info['basename']).'</span>').'</a>';
				}
			break;
		...
		if (!themerex_options_is_group($field['type']) && $field['type'] != 'hidden') {
			if ($cloneable) {
				echo '<div class="themerex_options_input_button themerex_options_clone_button themerex_options_clone_button_add">'. esc_html__('+ Add item', 'wineshop') .'</div>';
			}
			if (!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']))
				echo  '<div class="themerex_options_content_inherit"'.($inherit ? '' : ' style="display:none;"').'><div>'.esc_html__('Inherit', 'wineshop').'</div><input type="hidden" name="'.esc_attr($id).'_inherit" value="'.esc_attr($inherit).'" /></div>';
			if ( !in_array($field['type'], array('info', 'label', 'divider')))
				echo '</div>';
			if (!empty($field['desc']))
...
if ( !function_exists( 'themerex_options_save' ) ) {
	function themerex_options_save() {
		...
		if ($mode == 'save') {
			parse_str(themerex_get_value_gpc('data'), $post_data);
		} else if ($mode=='export') {
			parse_str(themerex_get_value_gpc('data'), $post_data);
...
if ( !function_exists( 'themerex_options_import' ) ) {
	function themerex_options_import() {
		...
		$override = themerex_get_value_gpc('override')=='' ? 'general' : themerex_get_value_gpc('override');
		...
		$response = array('error'=>$opt===false ? esc_html__('Error while unpack import data!', 'wineshop') : '', 'data'=>$opt);
		echo json_encode($response);
