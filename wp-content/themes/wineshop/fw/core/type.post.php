...
if ( !function_exists( 'themerex_post_settings_theme_setup2' ) ) {
	...
	function themerex_post_settings_theme_setup2() {
		themerex_add_theme_inheritance( array('archive' => array(
            'title' => esc_html__( 'Archive', 'wineshop' ),
            'stream_template_title' => esc_html__( 'Archive', 'wineshop' ),
            'single_template_title' => esc_html__( 'Archive', 'wineshop' ),
		...
		themerex_add_theme_inheritance( array('search' => array(
            ...
            'stream_template_title' => esc_html__( 'Search', 'wineshop' ),
            'single_template_title' => esc_html__( 'Search', 'wineshop' ),
		...
		themerex_add_theme_inheritance( array('page' => array(
            'title' => esc_html__( 'Page', 'wineshop' ),
            'stream_template_title' => esc_html__( 'Page', 'wineshop' ),
            'single_template_title' => esc_html__( 'Page', 'wineshop' ),
			'stream_template' => '',
		...
		if (isset($THEMEREX_GLOBALS['post_override_options']) && $THEMEREX_GLOBALS['post_override_options']['page']=='post') {
			$THEMEREX_GLOBALS['post_override_options']['fields'] = array(
					...
					"info_reviews_1" => array(
						"title" => esc_html__('Reviews criterias for this post', 'wineshop'),
						"override" => "post",
						"divider" => false,
						"desc" => wp_kses( __('In this section you can put your reviews marks', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
						"class" => "reviews_meta",
						"type" => "info"),
					"show_reviews" => array(
					...
					"reviews_marks" => array(
						...
						"override" => "post",
						"desc" => wp_kses( __("Marks for this review", 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
...
if (!function_exists('themerex_post_add_options_column')) {
	function themerex_post_add_options_column( $columns ){
		$columns['theme_options'] = esc_html__('Theme Options', 'wineshop');
		return $columns;
	}
}
			...
			if (is_array($props) && count($props) > 0) {
				foreach ($props as $prop_name=>$prop_value) {
					if (!themerex_is_inherit_option($prop_value) && (!isset($THEMEREX_GLOBALS['options'][$prop_name]['type']) || $THEMEREX_GLOBALS['options'][$prop_name]['type']!='hidden')) {
						...
						$options .= '<div class="themerex_options_prop_row"><span class="themerex_options_prop_name">' . esc_html($prop_title) . '</span>&nbsp;=&nbsp;<span class="themerex_options_prop_value">' . (is_array($prop_value) ? esc_html__('[Complex Data]', 'wineshop') : '"' . esc_html(themerex_strshort($prop_value, 80)) . '"') . '</span></div>';
					}
				}
			}
		}
...
if (!function_exists('themerex_post_admin_scripts')) {
	function themerex_post_admin_scripts() {
		global $THEMEREX_GLOBALS;
		if (function_exists('themerex_reviews_theme_setup') && isset($THEMEREX_GLOBALS['post_override_options']) && $THEMEREX_GLOBALS['post_override_options']['page']=='post' && themerex_storage_isset('options', 'show_reviews'))
...
if (!function_exists('themerex_post_before_show_override_options')) {
	function themerex_post_before_show_override_options($post_type, $post_id) {
        ...
        themerex_storage_set_array('js_vars', 'reviews_levels', themerex_get_theme_option('reviews_criterias_levels'));
        themerex_storage_set_array('js_vars', 'reviews_max_level', (int) $max_level);
        themerex_storage_set_array('js_vars', 'reviews_allow_user_marks', true);
        ?>
...
if ( !function_exists( 'themerex_is_post_page' ) ) {
	function themerex_is_post_page() {
		global $THEMEREX_GLOBALS, $wp_query;
		if (!empty($THEMEREX_GLOBALS['pre_query']))
			return $THEMEREX_GLOBALS['pre_query']->is_single() || $THEMEREX_GLOBALS['pre_query']->is_category() || $THEMEREX_GLOBALS['pre_query']->is_posts_page==1;
		else
...
if ( !function_exists( 'themerex_post_list_post_types' ) ) {
	function themerex_post_list_post_types($list) {
		$list['post'] = esc_html__('Post', 'wineshop');
		$list['page'] = esc_html__('Page', 'wineshop');
		return $list;
	}
}
...
if ( !function_exists( 'themerex_post_detect_inheritance_key' ) ) {
	function themerex_post_detect_inheritance_key($key) {
		...
		global $THEMEREX_GLOBALS;
		if (!empty($THEMEREX_GLOBALS['pre_query'])) {
			if ($THEMEREX_GLOBALS['pre_query']->is_day() || $THEMEREX_GLOBALS['pre_query']->is_month() || $THEMEREX_GLOBALS['pre_query']->is_year())
				$key = 'archive';
			else if ($THEMEREX_GLOBALS['pre_query']->is_search())
				$key = 'search';
			else if ($THEMEREX_GLOBALS['pre_query']->is_404())
				$key = 'error404';
			else if ($THEMEREX_GLOBALS['pre_query']->is_page())
				$key = 'page';
...
if ( !function_exists( 'themerex_post_get_blog_title' ) ) {
	function themerex_post_get_blog_title($title, $page) {
		...
		if ( $page == 'blog')
			$title = esc_html__( 'All Posts', 'wineshop' );
		else if ( $page == 'author' ) {
			$curauth = (get_query_var('author_name')) ? get_user_by('slug', get_query_var('author_name')) : get_userdata(get_query_var('author'));
			$title = sprintf(esc_html__('Author page: %s', 'wineshop'), $curauth->display_name);
		} else if ( $page == 'error404' )
			$title = esc_html__('URL not found', 'wineshop');
		else if ( $page == 'search' )
			$title = sprintf( esc_html__( 'Search: %s', 'wineshop' ), get_search_query() );
		else if ( $page == 'archives_day' )
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'wineshop' ), themerex_get_date_translations(get_the_date()) );
		else if ( $page == 'archives_month' )
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'wineshop' ), themerex_get_date_translations(get_the_date( 'F Y' )) );
		else if ( $page == 'archives_year' )
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'wineshop' ), get_the_date( 'Y' ) );
			...
			$title = sprintf( esc_html__( 'Attachment: %s', 'wineshop' ), themerex_get_post_title());
		else if ( $page == 'single' )
