...
if ( !function_exists( 'themerex_attachment_settings_theme_setup2' ) ) {
	...
	function themerex_attachment_settings_theme_setup2() {
		themerex_add_theme_inheritance( array('attachment' => array(
			...
			'stream_template_title' => esc_html__( 'Attachment', 'wineshop' ),
			'single_template_title' => esc_html__( 'Attachment', 'wineshop' ),
			'stream_template' => '',
...
if (!function_exists('themerex_attachment_theme_setup')) {
	...
	function themerex_attachment_theme_setup() {
		...
		if (function_exists('themerex_utils_require_data')) {
			themerex_utils_require_data( 'taxonomy', 'media_folder', array(
				'post_type'			=> array( 'attachment' ),
				'hierarchical' 		=> true,
				'labels' 			=> array(
					'name'              => esc_html__('Media Folders', 'wineshop'),
					'singular_name'     => esc_html__('Media Folder', 'wineshop'),
					'search_items'      => esc_html__('Search Media Folders', 'wineshop'),
					'all_items'         => esc_html__('All Media Folders', 'wineshop'),
					'parent_item'       => esc_html__('Parent Media Folder', 'wineshop'),
					'parent_item_colon' => esc_html__('Parent Media Folder:', 'wineshop'),
					'edit_item'         => esc_html__('Edit Media Folder', 'wineshop'),
					'update_item'       => esc_html__('Update Media Folder', 'wineshop'),
					'add_new_item'      => esc_html__('Add New Media Folder', 'wineshop'),
					'new_item_name'     => esc_html__('New Media Folder Name', 'wineshop'),
					'menu_name'         => esc_html__('Media Folders', 'wineshop'),
				),
				'query_var'			=> true,
				'rewrite' 			=> true,
	...
	function themerex_attachment_view_filters($settings, $post=null) {
		...
		if (is_array($taxes) && count($taxes) > 0) {
			foreach ($taxes as $tax) {
                ...
                if (is_array($terms) && count($terms) > 0) {
					$settings['mimeTypes'][$tax.'.'] = esc_html__('Media without folders', 'wineshop');
...
if (!function_exists('themerex_attachment_view_compat')) {
	function themerex_attachment_view_compat($form_fields, $post=null) {
		...
		if (isset($form_fields['media_folder'])) {
			...
			$html = '';
			$html .= '<input type="text" class="text" id="media_folder_'.esc_attr($id).'" name="media_folder___new" value="" placeholder="' . esc_attr__('Create new folder', 'wineshop') . '"' . ($readonly) . ' ' . ($aria_required) . ' />';
			if (is_array($terms) && count($terms) > 0) {
...
if ( !function_exists( 'themerex_attachmnent_detect_inheritance_key' ) ) {
	function themerex_attachmnent_detect_inheritance_key($key) {
		...
		return (!empty($THEMEREX_GLOBALS['pre_query']) ? $THEMEREX_GLOBALS['pre_query']->is_attachment() : is_attachment()) 
					? 'attachment' 
					: '';
	}
}
