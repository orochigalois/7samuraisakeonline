...
if (!function_exists('themerex_dcl')) {	function themerex_dcl($msg)	{ 	if (is_user_logged_in()) echo '<br>"' . esc_html($msg) . '"<br>'; } }		// Console log - output any message on the screen
if (!function_exists('themerex_dco')) {	function themerex_dco(&$var, $lvl=-1)	{ 	if (is_user_logged_in()) themerex_debug_dump_screen($var, $lvl); } }	// Console obj - output object structure on the screen
if (!function_exists('themerex_dcs')) {	function themerex_dcs($lvl=-1){ 	if (is_user_logged_in()) themerex_debug_calls_stack_screen($lvl); } }		// Console stack - output calls stack on the screen
if (!function_exists('themerex_dcw')) {	function themerex_dcw()		{	if (is_user_logged_in()) themerex_debug_dump_wp(); } }						// Console WP - output WP is_... states on the screen
if (!function_exists('themerex_ddo')) {	function themerex_ddo(&$var, $lvl=-1)	{ 	if (is_user_logged_in()) themerex_debug_dump_var($var, $lvl); } }		// Return obj - return object structure
if (!function_exists('themerex_dfl')) {	function themerex_dfl($var)	{	if (is_user_logged_in()) themerex_debug_trace_message($var); } }			// File log - output any message into file debug.log
if (!function_exists('themerex_dfo')) {	function themerex_dfo(&$var, $lvl=-1)	{ 	if (is_user_logged_in()) themerex_debug_dump_file($var, $lvl); } }	// File obj - output object structure into file debug.log
if (!function_exists('themerex_dfs')) {	function themerex_dfs($lvl=-1){ 	if (is_user_logged_in()) themerex_debug_calls_stack_file($lvl); } }			// File stack - output calls stack into file debug.log
