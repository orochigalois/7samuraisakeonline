...
if ( !function_exists( 'themerex_core_customizer_admin_menu_item' ) ) {
	function themerex_core_customizer_admin_menu_item() {
		themerex_admin_add_menu_item('submenu', array(
			...
			'page_title' => esc_html__('Fonts & Colors', 'wineshop'),
			'menu_title' => esc_html__('Fonts & Colors', 'wineshop'),
			'capability' => 'manage_options',
			'menu_slug'  => 'themerex_options_customizer',
			'callback'   => 'themerex_core_customizer_page',
			'icon'		 => ''
			)
		);
	}
...
if ( !function_exists( 'themerex_core_customizer_load_options' ) ) {
	function themerex_core_customizer_load_options() {
		...
		if ($mode!='reset' || $override!='customizer') {
			...
			$schemes = get_option( $THEMEREX_GLOBALS['theme_slug'] . '_options_custom_colors' );
			if (!empty($schemes)) $THEMEREX_GLOBALS['custom_colors'] = $schemes;
			$fonts = get_option( $THEMEREX_GLOBALS['theme_slug'] . '_options_custom_fonts' );
...
if ( !function_exists( 'themerex_core_customizer_save_options' ) ) {
	function themerex_core_customizer_save_options() {
		...
		parse_str(themerex_get_value_gpc('data'), $data);
		...
		update_option( $THEMEREX_GLOBALS['theme_slug'] . '_options_custom_colors', apply_filters('themerex_filter_save_custom_colors', $schemes));
		...
		update_option( $THEMEREX_GLOBALS['theme_slug'] . '_options_custom_fonts', apply_filters('themerex_filter_save_custom_fonts', $THEMEREX_GLOBALS['custom_fonts']));
	...
	function themerex_core_customizer_scheme_delete() {
		...
		if (isset($THEMEREX_GLOBALS['custom_colors'][$scheme])) {
			if (count($THEMEREX_GLOBALS['custom_colors']) > 1) {
				...
				$response['error'] = sprintf(esc_html__('You cannot delete last color scheme!', 'wineshop'), $scheme);
		} else
			$response['error'] = sprintf(esc_html__('Color Scheme %s not found!', 'wineshop'), $scheme);
			...
			foreach ($order as $slug) {
				if (is_array($THEMEREX_GLOBALS['custom_colors'][$slug]) && count($THEMEREX_GLOBALS['custom_colors'][$slug]) > 0) {
					...
					if ($slug == $scheme) {
						...
						$schemes[$new_slug]['title'] .= ' '.esc_html__('(Copy)', 'wineshop');
					}
				}
			}
			$THEMEREX_GLOBALS['custom_colors'] = $schemes;
	...
	function themerex_core_customizer_page() {
		...
		if (is_array($THEMEREX_GLOBALS['custom_colors']) && count($THEMEREX_GLOBALS['custom_colors']) > 0) {
			...
			$options["partition_schemes"] = array(
				"title" => esc_html__('Color schemes', 'wineshop'),
				"override" => "customizer",
			...
			foreach ($THEMEREX_GLOBALS['custom_colors'] as $slug=>$scheme) {
				...
				$options["{$slug}-description"] = array(
					"title" => sprintf(esc_html__('Color scheme "%s"', 'wineshop'), $scheme['title']),
					"desc" => wp_kses( sprintf(__('Specify the color for each element in the scheme "%s". After that you will be able to use your color scheme for the entire page, any part thereof and/or for the shortcodes!', 'wineshop'), $scheme['title']), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-buttons_label"] = array(
					"desc" => wp_kses( __("You can duplicate current color scheme (appear on new tab) or delete it (if not last scheme)", 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "4_6 first",
					"type" => "label");
				...
				$options["{$slug}-button_copy"] = array(
					"title" => esc_html__('Copy',  'wineshop'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_6",
					"icon" => "iconadmin-docs",
				...
				$options["{$slug}-button_delete"] = array(
					"title" => esc_html__('Delete',  'wineshop'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_6 last",
				...
				$options["{$slug}-slug"] = array(
					"title" => esc_html__('Slug',  'wineshop'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_5 last",
					"std" => "",
				...
				if ($demo_block) {
					$options["{$slug}-demo"] = array(
						"title" => esc_html__('Usage demo', 'wineshop'),
						"desc" => wp_kses( __('Below you can see the example of decoration of the page with selected colors.', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] )
									. trim($demo_block),
						"override" => "customizer",
						"type" => "info");
				}
...
if (isset($scheme['accent1'])) {
				$options["{$slug}-accent_info"] = array(
					"title" => esc_html__('Accent colors', 'wineshop'),
					"desc" => wp_kses( __('Specify colors for theme accented elements (if need). The theme may not use all of the colors.', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"type" => "info");

				// Accent 1 color
				$options["{$slug}-accent1_label"] = array(
					"title" => esc_html__('Accent 1', 'wineshop'),
					"desc" => wp_kses( __('Select color for accented elements and their hover state', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-accent1"] = array(
					"title" => esc_html__('Color', 'wineshop'),
					"std" => "#B4BB6B",
					"val" => $scheme['accent1'],
					"override" => "customizer",
					"divider" => false,
				...
				$options["{$slug}-accent1_hover"] = array(
					"title" => esc_html__('Hover', 'wineshop'),
					"std" => "#2B2B2B",
...
if (isset($scheme['accent2'])) {
				...
				$options["{$slug}-accent2_label"] = array(
					"title" => esc_html__('Accent 2', 'wineshop'),
					"desc" => wp_kses( __('Select color for accented elements and their hover state', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
...
if (isset($scheme['accent3'])) {
				...
				$options["{$slug}-accent3_label"] = array(
					"title" => esc_html__('Accent 3', 'wineshop'),
					"desc" => wp_kses( __('Select color for accented elements and their hover state', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
...
if (isset($scheme['text'])) {
				...
				$options["{$slug}-text_info"] = array(
					"title" => esc_html__('Text and Headers', 'wineshop'),
					"desc" => wp_kses( __('Specify colors for the plain text, post info blocks and headers', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-text_label"] = array(
					...
					"desc" => wp_kses( __('Select colors for the text: normal text color, light text (for example - post info) and dark text (headers, bold text, etc.)', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-text_light"] = array(
					"title" => esc_html__('Light', 'wineshop'),
					"std" => "",
				...
				$options["{$slug}-text_dark"] = array(
					"title" => esc_html__('Dark', 'wineshop'),
					"std" => "",
...
if (isset($scheme['inverse_text'])) {
				...
				$options["{$slug}-inverse_label"] = array(
					"title" => esc_html__('Inverse text', 'wineshop'),
					"desc" => wp_kses( __('Select colors for inversed text (text on accented background)', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-inverse_labe2l"] = array(
					"title" => esc_html__('Inverse links', 'wineshop'),
					"desc" => wp_kses( __('Select colors for inversed links (links on accented background)', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['bg_color'])) {
				...
				$options["{$slug}-block_info"] = array(
					"title" => esc_html__('Page/Block decoration', 'wineshop'),
					"desc" => wp_kses( __('Specify border and background to decorate whole page (if scheme accepted to the page) or entire block/section.', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"type" => "info");
	
				// Border
				$options["{$slug}-bd_color_label"] = array(
					"title" => esc_html__('Border color', 'wineshop'),
					"desc" => wp_kses( __('Select the border color and it hover state', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_color_label"] = array(
					...
					"desc" => wp_kses( __('Select the background color and it hover state', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['bg_image'])) {
				...
				$options["{$slug}-bg_image_label"] = array(
					"title" => esc_html__('Background image', 'wineshop'),
					"desc" => wp_kses( __('Select first background image and it display parameters', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_image_repeat"] = array(
					"title" => esc_html__('Repeat', 'wineshop'),
					"std" => "",
				...
				$options["{$slug}-bg_image_attachment"] = array(
					"title" => esc_html__('Attachment', 'wineshop'),
					"std" => "",
				...
				$options["{$slug}-bg_image2_label"] = array(
					"title" => esc_html__('Background image 2', 'wineshop'),
					"desc" => wp_kses( __('Select second background image and it display parameters', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['alter_text'])) {
				...
				$options["{$slug}-alter_info"] = array(
					"title" => esc_html__('Alternative colors: Highlight areas / Input fields', 'wineshop'),
					"desc" => wp_kses( __('Specify colors to decorate inner blocks or input fields in the forms', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-alter_text_label"] = array(
					"title" => esc_html__('Text and Headers', 'wineshop'),
					"desc" => wp_kses( __('Specify colors for the plain text, post info blocks and headers in the highlight blocks', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_link_label"] = array(
					...
					"desc" => wp_kses( __('Specify colors for the links in the highlight blocks', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bd_color_label"] = array(
					"title" => esc_html__('Border color', 'wineshop'),
					"desc" => wp_kses( __('Select the border colors for the normal state and for active (focused) field', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bg_color_label"] = array(
					"title" => esc_html__('Background Color', 'wineshop'),
					"desc" => wp_kses( __('Select the background colors for the normal state and for active (focused) field', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bg_image_label"] = array(
					"title" => esc_html__('Background image', 'wineshop'),
					"desc" => wp_kses( __('Select alter background image and it display parameters', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
		...
		if (is_array($THEMEREX_GLOBALS['custom_fonts']) && count($THEMEREX_GLOBALS['custom_fonts']) > 0) {
			...
			$options["partition_fonts"] = array(
				"title" => esc_html__('Fonts', 'wineshop'),
				"override" => "customizer",
				"icon" => "iconadmin-font",
				"type" => "partition");
			if ($start_partition) {
			...
			$list_weight = array(
				'inherit' => esc_html__("Inherit", 'wineshop'),
				'100' => esc_html__('100 (Light)', 'wineshop'),
				'300' => esc_html__('300 (Thin)',  'wineshop'),
				'400' => esc_html__('400 (Normal)', 'wineshop'),
				'500' => esc_html__('500 (Semibold)', 'wineshop'),
				'600' => esc_html__('600 (Semibold)', 'wineshop'),
				'700' => esc_html__('700 (Bold)', 'wineshop'),
				'900' => esc_html__('900 (Black)', 'wineshop')
			);
			...
			foreach ($THEMEREX_GLOBALS['custom_fonts'] as $slug=>$font) {
				...
				if (isset($font['font-size'])) {
					$options["{$slug}-font-size"] = array(
						"title" => $show_titles ? esc_html__('Size', 'wineshop') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['line-height'])) {
					$options["{$slug}-line-height"] = array(
						"title" => $show_titles ? esc_html__('Line height', 'wineshop') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['font-weight'])) {
					$options["{$slug}-font-weight"] = array(
						"title" => $show_titles ? esc_html__('Weight', 'wineshop') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['font-style'])) {
					$options["{$slug}-font-style"] = array(
						"title" => $show_titles ? esc_html__('Style', 'wineshop') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['margin-top'])) {
					$options["{$slug}-margin-top"] = array(
						"title" => $show_titles ? esc_html__('Margin Top', 'wineshop') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['margin-bottom'])) {
					$options["{$slug}-margin-bottom"] = array(
						"title" => $show_titles ? esc_html__('Margin Bottom', 'wineshop') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
		...
		themerex_options_page_start(array(
			'title' => esc_html__('Fonts & Colors', 'wineshop'),
			"icon" => "iconadmin-cog",
			"subtitle" => esc_html__('Fonts settings & Color schemes', 'wineshop'),
			"description" => wp_kses( __('Customize fonts and colors for your site.', 'wineshop'), $THEMEREX_GLOBALS['allowed_tags'] ),
			'data' => $options,
			'create_form' => true,
...
if (!function_exists('themerex_core_customizer_load_scripts')) {
	function themerex_core_customizer_load_scripts() {
		...
		if (file_exists(themerex_get_file_dir('core/core.customizer/core.customizer.js')))
            wp_enqueue_script( 'themerex-core-customizer-script', themerex_get_file_url('core/core.customizer/core.customizer.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'themerex_core_customizer_prepare_scripts' ) ) {
	function themerex_core_customizer_prepare_scripts() {
        themerex_storage_merge_array('js_vars', 'to_strings', array(
            'scheme_delete' => esc_html__("Delete color scheme", 'wineshop'),
            'scheme_delete_confirm' => esc_html__("Do you really want to delete this color scheme?", 'wineshop'),
            'scheme_delete_complete' => esc_html__("Current color scheme is successfully deleted!", 'wineshop'),
            'scheme_delete_failed' => esc_html__("Error while delete color scheme! Try again later.", 'wineshop'),
            'scheme_copy' => esc_html__("Copy color scheme", 'wineshop'),
            'scheme_copy_confirm' => esc_html__("Duplicate this color scheme?", 'wineshop'),
            'scheme_copy_complete' => esc_html__("Current color scheme is successfully duplicated!", 'wineshop'),
            'scheme_copy_failed' => esc_html__("Error while duplicate color scheme! Try again later.", 'wineshop')
        ));
	}
}
