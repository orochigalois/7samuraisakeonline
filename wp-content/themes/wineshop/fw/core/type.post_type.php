	...
	function themerex_post_type_theme_setup() {
		...
		if ($override_key) {
			...
			$THEMEREX_GLOBALS['post_override_options'] = array(
				...
				'title' => esc_html__('Post Options', 'wineshop'),
				'page' => $post_type,
				'context' => 'normal',
				'priority' => 'high',
				'fields' => array()
...
if (!function_exists('themerex_post_type_add_override_options')) {
    ...
    function themerex_post_type_add_override_options($boxes = array()) {
        $boxes[] = array_merge(themerex_storage_get('post_override_options'), array('callback' => 'themerex_post_type_show_override_options'));
        return $boxes;
    }
}
...
if (!function_exists('themerex_post_type_show_override_options')) {
	function themerex_post_type_show_override_options() {
		...
		echo '<input type="hidden" name="override_options_post_nonce" value="' .esc_attr($THEMEREX_GLOBALS['admin_nonce']).'" />';
		...
		if (is_array($post_options) && count($post_options) > 0) {
			foreach ($post_options as $id=>$option) { 
				...
				$meta = isset($custom_options[$id]) 
								? apply_filters('themerex_filter_post_show_custom_field_value', $custom_options[$id], $option, $id, $post_type, $post->ID) 
...
if (!function_exists('themerex_post_type_save_options')) {
	function themerex_post_type_save_options($post_id) {
		...
		// verify nonce
		if (!isset($_POST['override_options_post_nonce']) || !wp_verify_nonce($_POST['override_options_post_nonce'], $THEMEREX_GLOBALS['admin_url'])) {
			return $post_id;
		}
		...
		$post_type = isset($_POST['override_options_post_type']) ? themerex_get_value_gpc('override_options_post_type') : themerex_get_value_gpc('post_type');
		$override_key = themerex_get_override_key($post_type, 'post_type');
